...
class Network(MyModel):
    ...
    def __str__(self):
        return "%s/%s - %s, %s" % (self.name, self.cidr, self.rg, self.type)
...
def create_ip_addr(sender, instance:Network, created, **kwargs):
     if created:
        ...
        for ipn in subnet.hosts():
            ip1=IPAddr(name=ipn,dns_name="blank",owner=instance.owner, net=instance)
            ip1.save()
...
class IPAddr(MyModel):
    ...
    def __str__(self):
        return "%s (%s): %s/%s" % (self.name, self.dns_name, self.net.name, self.net.cidr)
...
class NetPCInt(MyModel):
    ...
    #ip = models.ForeignKey(IPAddr, on_delete=models.PROTECT,blank=True)
    ip = models.ManyToManyField(IPAddr,blank=True, related_name="ipaddr")

    def __str__(self):
        return "%s (%s) : %s" % (self.net_obj.name, self.name, self.ip)
